...
void add_castling(struct ChessBoard *board, unsigned char *color_pos, short *color_first_move, unsigned char piece_ind, signed char *moves, short* moves_count){
    ...
    if(color_first_move[piece_ind] == -1 && color_first_move[board->king_pos] == -1){
...
void add_en_passant(struct ChessBoard *board, unsigned char *color_pos,unsigned char piece_pos_ind, signed char *moves, short *moves_count){
    if (board->move_count > 0){
        if (board->past_moves[board->move_count-1] == DOUBLE_PAWN){
            if (color_pos[piece_pos_ind+1] == board->past_moves[board->move_count-2] && abs(color_pos[piece_pos_ind] - board->past_moves[board->move_count-3]) == 1){
                add_move(color_pos[piece_pos_ind], color_pos[piece_pos_ind+1], board->past_moves[board->move_count-3], board->past_moves[board->move_count-2] + board->color_to_move, EN_PASSANT, moves, moves_count);
            }
        }
    }
}
